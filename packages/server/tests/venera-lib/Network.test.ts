...
assert.ok(
	"GET",
	Network.get(HTTP_BIN_URL + "/get").then(v => v.status === 200)
);
assert.ok(
	"POST",
	Network.post(HTTP_BIN_URL + "/post").then(v => v.status === 200)
);
assert.ok(
	"PATCH",
	Network.patch(HTTP_BIN_URL + "/patch").then(v => v.status === 200)
);
assert.ok(
	"DELETE",
	Network.delete(HTTP_BIN_URL + "/delete").then(v => v.status === 200)
);
assert.ok(
	"PUT",
	Network.put(HTTP_BIN_URL + "/put").then(v => v.status === 200)
);

assert.eq(
	"POST with headers",
	Network.post(HTTP_BIN_URL + "/post", { "X-Puraty-H1": "Value" }).then(v => {
...
assert.ok("Get cookies", async () => {
	...
	return (
		cookies.find(v => v.name === "test")?.value === "value" &&
		cookies.find(v => v.name === "test2")?.value === "value2"
	);
});
assert.ok("Request with cookies", async () => {
	const resp = JSON.parse(
		(await Network.get(HTTP_BIN_URL + "/cookies")).body
	).cookies;
...
assert.ok("Request without cookies", async () => {
	...
	return (
		typeof resp?.test === "undefined" && typeof resp?.test2 === "undefined"
	);
});
