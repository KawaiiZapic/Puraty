...
assert.eq("query selector for non exist element", document.querySelector("h1"), null);
assert.eq("query selector all for non exist element", document.querySelectorAll("h1").length, 0);
...
assert.eq("nextEl should return null when there is not next el", child.nextElementSibling?.nextElementSibling, null);
assert.eq("prevEl should return null when there is not prev el", child.previousElementSibling?.previousElementSibling, null);