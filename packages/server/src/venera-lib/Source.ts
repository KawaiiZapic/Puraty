...
interface SearchOptions {
  ...
  load(keyword: string, options: string[], page: number): Promise<{comics: Comic[], maxPage: number}>;
  loadNext(keyword: string, options: string[], next: string | null): Promise<{comics: Comic[], maxPage: number}>;
}
...
interface ComicLoader {
  ...
  loadThumbnails?(id: string, next: string | null): Promise<{ thumbnails: string[], next?: string }>;
  starRating?(id: string, rating: number): Promise<unknown>;
  loadEp(comicId: string, epId?: string): Promise<{ images: string[] }>;
  onImageLoad?(url: string, comicId: string, epId?: string): Promise<ImageLoadingConfig>;
  onThumbnailLoad?(url: string): Promise<ImageLoadingConfig>;
  likeComic?(id: string, isLike: boolean): Promise<unknown>;
  loadComments?(comicId: string, subId: string | null, page: number, replyTo?: string): Promise<{ comments: Comment[], maxPage?: number }>;
  sendComment?(comicId: string, subId: string | null, content: string, replyTo?: string): Promise<unknown>;
  likeComment?(comicId: string, subId: string | null, commentId: string, isLike: boolean): Promise<unknown>;
  voteComment?(comicId: string, subId: string | null, commentId: string, isUp: boolean, isCancel: boolean): Promise<unknown>;
  idMatch?: string;
