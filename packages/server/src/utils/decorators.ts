...
const setParamInfo = (func: unknown, idx: number, info: ParamInfo | { required: boolean }) => {
  let { params } = initControllerInfo(func);
  if (!params[idx]) {
    params[idx] = {
      ...info as any
    }
      ...
      const args: unknown[] = await Promise.all(info.params.map(async (v) => {
        if (v.type === "event") {
          ...
          try {
            ...
            if (v.required) {
              throw HTTPError.status(400, undefined, { message: "Required parameter JSON@body not existed" });
            }
          }
        } else if (v.type === "path") {
          const r = e.context.params?.[v.id];
          if (v.required && typeof r === "undefined") {
            throw HTTPError.status(400, undefined, { message: "Required parameter " + v.id + "@path not existed" });
          }
          return r;
        } else if (v.type === "query") {
          const r = getQuery(e)[v.id];
          if (v.required && typeof r === "undefined") {
            throw HTTPError.status(400, undefined, { message: "Required parameter " + v.id + "@query not existed" });
          }
          return r;
        }
      }));
